...
char *sdoc[] = {
    /**/
    "                                                                                     ",
    " SUFDMOD2DTTIELASTIC.V4.0 nz= nx= nt= dx= dz= dt= fpeak= ...                         ",
    " Rotated staggered grid finite difference simulation of two-dimensional              ",
    " first-order elastic wave equation (Bond Transformation) in TTI media                ",
...
int main(int argc, char *argv[])
{
  ...
  float **invrho = NULL, **c11 = NULL, **c13 = NULL, **c15 = NULL, **c33 = NULL, **c35 = NULL, **c55 = NULL;
...
#pragma omp parallel for private(ix, iz, C11, C13, C33, C55, f, sin2_theta, cos2_theta, sin4_theta, cos4_theta,      \
...
#pragma ivdep
    for (iz = 0; iz < Nz; iz++)
    {
      ...
      C11 = rho[ix][iz] * vp[ix][iz] * vp[ix][iz] * (1.0 + 2.0 * epsilon[ix][iz]);
      C13 = rho[ix][iz] * (vp[ix][iz] * vp[ix][iz] * sqrt(f * (f + 2.0 * delta[ix][iz])) - vs[ix][iz] * vs[ix][iz]);
      C33 = rho[ix][iz] * vp[ix][iz] * vp[ix][iz];
      C55 = rho[ix][iz] * vs[ix][iz] * vs[ix][iz];

      theta[ix][iz] = scalar * theta[ix][iz] / 180.0 * PI;

      sin2_theta = sin(theta[ix][iz]) * sin(theta[ix][iz]);
      cos2_theta = cos(theta[ix][iz]) * cos(theta[ix][iz]);
      sin4_theta = sin2_theta * sin2_theta;
      cos4_theta = cos2_theta * cos2_theta;
      sin_2theta = sin(2.0 * theta[ix][iz]);
      cos_2theta = cos(2.0 * theta[ix][iz]);
      sin2_2theta = sin_2theta * sin_2theta;
      cos2_2theta = cos_2theta * cos_2theta;

      c11[ix][iz] = C11 * cos4_theta + C33 * sin4_theta + (0.5 * C13 + C55) * sin2_2theta;
      c13[ix][iz] = (0.25 * (C11 + C33) - C55) * sin2_2theta + C13 * (sin4_theta + cos4_theta);
      c15[ix][iz] = 0.5 * (-1.0 * C11 * cos2_theta + C33 * sin2_theta + (C13 + 2.0 * C55) * cos_2theta) * sin_2theta;
      c33[ix][iz] = C11 * sin4_theta + C33 * cos4_theta + (0.5 * C13 + C55) * sin2_2theta;
      c35[ix][iz] = 0.5 * (-1.0 * C11 * sin2_theta + C33 * cos2_theta - (C13 + 2.0 * C55) * cos_2theta) * sin_2theta;
      c55[ix][iz] = 0.25 * (C11 + C33 - 2.0 * C13) * sin2_2theta + C55 * cos2_2theta;
    }
  }
...
#pragma ivdep
  ...
  if (freesurface == 1)
  { /* improve the model parameters at free-surface */
    iz = ealThick;
#pragma omp parallel for private(ix) firstprivate(iz)
    for (ix = 0; ix < Nx; ix++)
    {
      c35[ix][iz] = -(c13[ix][iz] * c55[ix][iz] - c15[ix][iz] * c35[ix][iz]) /
                    (c33[ix][iz] * c55[ix][iz] - c35[ix][iz] * c35[ix][iz]);
      c55[ix][iz] = (c13[ix][iz] * c35[ix][iz] - c15[ix][iz] * c33[ix][iz]) /
                    (c33[ix][iz] * c55[ix][iz] - c35[ix][iz] * c35[ix][iz]);
      if (isnan(c35[ix][iz]) || isinf(c35[ix][iz]))
      {
        c35[ix][iz] = -c13[ix][iz] / c33[ix][iz];
      }
      if (isnan(c35[ix][iz]) || isinf(c35[ix][iz]))
      {
        c35[ix][iz] = 0.0f;
      }
      c11[ix][iz] = c11[ix][iz] - c13[ix][iz] * c35[ix][iz] + c15[ix][iz] * c55[ix][iz];
      c13[ix][iz] = 0.0f;
      c15[ix][iz] = 0.0f;
      c33[ix][iz] = 0.0f;
      c35[ix][iz] = 0.0f;
  ...
  efclose(fp);
  sprintf(fileName, "c15_%05d_%05d.bin", Nz, Nx);
  fp = fopen(fileName, "wb");
  efwrite(c15[0], FSIZE, Np, fp);
  efclose(fp);
  sprintf(fileName, "c33_%05d_%05d.bin", Nz, Nx);
  fp = fopen(fileName, "wb");
  efwrite(c33[0], FSIZE, Np, fp);
  efclose(fp);
  sprintf(fileName, "c35_%05d_%05d.bin", Nz, Nx);
  fp = fopen(fileName, "wb");
  efwrite(c35[0], FSIZE, Np, fp);
  efclose(fp);
  ...
  float dsigma_xxdx, dsigma_zzdz;
  float dsigma_xzdx, dsigma_xzdz;
  float invdx, invdz;
...
#pragma omp parallel for private(ix, iz, dsigma_xxdx, dsigma_zzdz, dsigma_xzdx, dsigma_xzdz, d1, d2)                 \
...
#pragma ivdep
      for (iz = izStart; iz < izEnd; iz++)
      {
        ...
        vx[ix][iz] = coef1[ix][iz] * vx[ix][iz] + coef2[ix][iz] * invrho[ix][iz] * (dsigma_xxdx + dsigma_xzdz);
        vz[ix][iz] = coef1[ix][iz] * vz[ix][iz] + coef2[ix][iz] * invrho[ix][iz] * (dsigma_xzdx + dsigma_zzdz);
      }
    }
    /********* Load Source *********/
    if (stype == 2 || stype == 4)
...
#pragma ivdep
      for (iz = izStart; iz < izEnd; iz++)
      {
        ...
        sigma_xx[ix][iz] =
            coef1[ix][iz] * sigma_xx[ix][iz] +
            coef2[ix][iz] * (c11[ix][iz] * dvxdx + c13[ix][iz] * dvzdz + c15[ix][iz] * (dvzdx + dvxdz));
        sigma_zz[ix][iz] =
            coef1[ix][iz] * sigma_zz[ix][iz] +
            coef2[ix][iz] * (c13[ix][iz] * dvxdx + c33[ix][iz] * dvzdz + c35[ix][iz] * (dvzdx + dvxdz));
        sigma_xz[ix][iz] =
            coef1[ix][iz] * sigma_xz[ix][iz] +
            coef2[ix][iz] * (c15[ix][iz] * dvxdx + c35[ix][iz] * dvzdz + c55[ix][iz] * (dvzdx + dvxdz));
      }
    }
