...
char *sdoc[] = {
    /**/
    "                                                                                     ",
    " SUFDMOD2DTTIELASTIC.V3.0 nz= nx= nt= dx= dz= dt= fpeak= ...                         ",
    " Rotated staggered grid finite difference simulation of two-dimensional              ",
    " first-order elastic wave equation (Rotated Tensor) in TTI media                     ",
...
int main(int argc, char *argv[])
{
  ...
  float **sin2_theta = NULL, **cos2_theta = NULL, **sin_2theta = NULL;
  float **vx = NULL, **vz = NULL;
  float **sigma_xx = NULL, **sigma_zz = NULL, **sigma_xz = NULL;
  float **tau_xx = NULL, **tau_zz = NULL, **tau_xz = NULL;
  ...
  sin2_theta = alloc2float(Nz, Nx);
  cos2_theta = alloc2float(Nz, Nx);
  sin_2theta = alloc2float(Nz, Nx);
  vx = alloc2float(Nz, Nx);
  vz = alloc2float(Nz, Nx);
  sigma_xx = alloc2float(Nz, Nx);
  sigma_zz = alloc2float(Nz, Nx);
  sigma_xz = alloc2float(Nz, Nx);
  tau_xx = alloc2float(Nz, Nx);
  tau_zz = alloc2float(Nz, Nx);
  tau_xz = alloc2float(Nz, Nx);
  D = alloc2float(Nz, Nx);
...
#pragma ivdep
    for (iz = 0; iz < Nz; iz++)
    {
      ...
      sin2_theta[ix][iz] = sin(theta[ix][iz]) * sin(theta[ix][iz]);
      cos2_theta[ix][iz] = cos(theta[ix][iz]) * cos(theta[ix][iz]);
      sin_2theta[ix][iz] = sin(2.0 * theta[ix][iz]);
    }
  }
...
#pragma ivdep
  ...
  memset(tau_xx[0], 0, Np * FSIZE);
  memset(tau_zz[0], 0, Np * FSIZE);
  memset(tau_xz[0], 0, Np * FSIZE);
  if (energy == 1)
  {
    memset(E, 0, nt * FSIZE);
...
#pragma omp parallel for private(isrc, isx, isz) firstprivate(dt, invdx, invdz, ealThick)
    ...
    /********* finite difference scheme *********/
#pragma omp parallel for private(ix, iz, dvxdx, dvxdz, dvzdx, dvzdz, d1, d2, dexxdt, dezzdt, dexzdt)                 \
    firstprivate(invdx, invdz, C1, C2, C3, C4, C5)
    for (ix = ixStart; ix < ixEnd; ix++)
    {
#pragma ivdep
      for (iz = izStart; iz < izEnd; iz++)
      {
        ...
        dexxdt =
            cos2_theta[ix][iz] * dvxdx + sin2_theta[ix][iz] * dvzdz - 0.5f * sin_2theta[ix][iz] * (dvxdz + dvzdx);
        dezzdt =
            sin2_theta[ix][iz] * dvxdx + cos2_theta[ix][iz] * dvzdz + 0.5f * sin_2theta[ix][iz] * (dvxdz + dvzdx);
        dexzdt = sin_2theta[ix][iz] * (dvxdx - dvzdz) + (cos2_theta[ix][iz] - sin2_theta[ix][iz]) * (dvxdz + dvzdx);

        tau_xx[ix][iz] =
            coef1[ix][iz] * tau_xx[ix][iz] + coef2[ix][iz] * (c11[ix][iz] * dexxdt + c13[ix][iz] * dezzdt);
        tau_zz[ix][iz] =
            coef1[ix][iz] * tau_zz[ix][iz] + coef2[ix][iz] * (c13[ix][iz] * dexxdt + c33[ix][iz] * dezzdt);
        tau_xz[ix][iz] = coef1[ix][iz] * tau_xz[ix][iz] + coef2[ix][iz] * c55[ix][iz] * dexzdt;
      }
    }
#pragma omp parallel for private(ix, iz)
    for (ix = ixStart; ix < ixEnd; ix++)
...
#pragma ivdep
      for (iz = izStart; iz < izEnd; iz++)
      {
        sigma_xx[ix][iz] = cos2_theta[ix][iz] * tau_xx[ix][iz] + sin2_theta[ix][iz] * tau_zz[ix][iz] +
                           sin_2theta[ix][iz] * tau_xz[ix][iz];
        sigma_zz[ix][iz] = sin2_theta[ix][iz] * tau_xx[ix][iz] + cos2_theta[ix][iz] * tau_zz[ix][iz] -
                           sin_2theta[ix][iz] * tau_xz[ix][iz];
        sigma_xz[ix][iz] = -0.5f * sin_2theta[ix][iz] * (tau_xx[ix][iz] - tau_zz[ix][iz]) +
                           (cos2_theta[ix][iz] - sin2_theta[ix][iz]) * tau_xz[ix][iz];
      }
    }
...
#pragma ivdep
          for (iz = MAX(isz - N, 0); iz <= MIN(isz + N, Nz - 1); iz++)
          {
            ...
            tau_xx[ix][iz] += 0.5f * weight_src * source.src[isrc][it];
            tau_zz[ix][iz] += 0.5f * weight_src * source.src[isrc][it];
          }
        }
      }
    }
...
#pragma omp parallel for reduction(+ : ek, ep) private(ix, iz, ixx, izz) firstprivate(ealThick)
  ...
  free2float(c33);
  free2float(c55);
  free2float(sin2_theta);
  free2float(cos2_theta);
  free2float(sin_2theta);
  free2float(vx);
  free2float(vz);
  free2float(sigma_xx);
  free2float(sigma_zz);
